...
func StartRedisPoller(ctx context.Context, db *gorm.DB, redisClient *redis.Client) {
	...
	for {
		...
		case <-ticker.C:
			...
			handleStageTriggers(ctx, now, db, redisClient)
		case <-ctx.Done():
			return
		}
	}
...
func handleStageTriggers(ctx context.Context, now int64, db *gorm.DB, redisClient *redis.Client) {
	...
	for _, stageKey := range stages {
		...
		if n, _ := redisClient.ZRem(ctx, "job-trigger", stageKey.Member).Result(); n > 0 {
			...
			go ExecuteJob(stageId, workflowId, db, redisClient)
		}
	}
}
